...
export const removeMCPServerData = (
  ...
  server.tool(
    ...
    "Clear client data from MCP server and revoke access",
    async () => {
      ...
      if (response.ok) {
        return {
          content: [
            {
              type: "text",
              text: "The MCP server data was removed. 1. Close your MCP client. 2. Clear cache `rm -rf ~/.mcp-auth`. 3. Open MCP client again to re-authenticate.",
            },
          ],
        };
      }
      ...
      return createErrorResponse(
        `Error removing client data in ${TAG_MANAGER_REMOVE_MCP_SERVER_DATA}`,
        "Unknown error",
      );
    },
  );
